...
var url        = system.args[1],
    ...
    globalBeforeOpenJS = system.args[7],
    pathBeforeOpenJS = system.args[8],
...
globalBeforeOpenJS = globalBeforeOpenJS === 'false' ? false : globalBeforeOpenJS;
pathBeforeOpenJS   = pathBeforeOpenJS === 'false' ? false : pathBeforeOpenJS;
...
runInitJavaScriptThen(function () {
...
function runInitJavaScriptThen(callback) {
  initJavaScriptRan = true;
  if (globalBeforeOpenJS && pathBeforeOpenJS) {
    require(globalBeforeOpenJS)(page, function thenExecuteOtherBeforeOpenFile() {
      require(pathBeforeOpenJS)(page, callback);
    });
  }
  else if (globalBeforeOpenJS) {
    require(globalBeforeOpenJS)(page, callback);
  }
  else if (pathBeforeOpenJS) {
    require(pathBeforeOpenJS)(page, callback);
  }
  else {
    callback();
